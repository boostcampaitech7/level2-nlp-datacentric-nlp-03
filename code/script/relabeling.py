...
class LabelErrorCorrector:
    ...
    def load_data(self, train_path_or_data, relabeling_path_or_data):
        """학습 및 테스트 데이터를 로드합니다."""
        # 학습 데이터 로드
        if isinstance(train_path_or_data, str):
            try:
            ...
            self.train_df = train_path_or_data

        # 테스트 데이터 로드
        if isinstance(relabeling_path_or_data, str):
            try:
                self.test_df = pd.read_csv(relabeling_path_or_data)
            except FileNotFoundError as e:
                print(f"파일을 찾을 수 없습니다: {e}")
                raise
        else:
            self.test_df = relabeling_path_or_data
    ...
    def corrected_data(self, output_path="corrected_train_data.csv"):
        """수정된 라벨 데이터를 저장합니다."""
    ...
    def run(self, train_path_or_data, relabeling_path_or_data, output_path="corrected_train_data.csv"):
        """전체 파이프라인을 실행합니다."""
        self.load_data(train_path_or_data, relabeling_path_or_data)
        self.train_model()
        self.detect_label_errors()
        final_data = self.corrected_data(output_path)
...
if __name__ == "__main__":
    ...
    lec.run(
        ...
        relabeling_path_or_data="/data/ephemeral/home/Yunseo_DCTC/data/train.csv",
